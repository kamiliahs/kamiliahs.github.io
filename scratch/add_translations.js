...
let content = fs.readFileSync('src/js/modules/i18n.js', 'utf8');

for (const [lang, translations] of Object.entries(missing)) {
    // Find the end of the language block
    // We can look for `lang_pt: '...'` which is at the end of each block
    const searchStr = `lang_pt: '${
        lang === 'es' ? 'Portugués' : 
        lang === 'en' ? 'Portuguese' : 
        lang === 'nl' ? 'Portugees' : 
        'Portugais'
...
fs.writeFileSync('src/js/modules/i18n.js', content);
